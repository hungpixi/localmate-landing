...
Last updated May 31, 2026 [ View as Markdown](https://flueframework.com/docs/sdk/overview/index.md) 
...
Admin APIs use the origin-relative read-only mount path configured with `adminBasePath`, independently from the public `baseUrl` pathname. This option only tells the SDK where an already-mounted `admin()` sub-app lives. The application must [mount admin() explicitly and protect that mount with application-owned authorization](https://flueframework.com/docs/api/routing-api/#admin).
...
Current page: [SDK API](https://flueframework.com/docs/sdk/overview/)
...
* [ SDK API ](https://flueframework.com/docs/sdk/overview/)