      ...
      <div style={{ backgroundColor: '#ffffff', padding: '5rem 0', textAlign: 'center' }}>
        <Container size="md">
          ...
          <p style={{ color: 'var(--color-text-muted)', margin: '1rem 0 2rem 0' }}>
            Vui lòng quay lại trang danh mục dịch vụ để khám phá các gói giải pháp khác của LocalMate.
          </p>
    ...
    <div style={{ backgroundColor: '#ffffff', padding: '2rem 0 5rem 0' }}>
      ...
      <Container size="lg">
        ...
        >
          <div>
            {service.badge && (
              ...
              >
                <Sparkles size={14} color="var(--color-teal)" /> {service.badge}
              </span>
            )}
            ...
            >
              ...
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', display: 'block', fontWeight: 600 }}>Thời gian triển khai</span>
                <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-navy)' }}>{service.sla}</span>
              </div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '3.5rem' }}>
          ...
          >
            <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--color-navy)', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
              <CheckCircle2 size={22} color="var(--color-teal-dark)" /> Phù hợp nhất với:
            </h3>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              {service.suitableFor.map((item, idx) => (
                <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', fontSize: '0.925rem', color: 'var(--color-navy)', lineHeight: 1.55 }}>
        ...
        >
          <div style={{ textAlign: 'center', maxWidth: '700px', margin: '0 auto 2.5rem auto' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--color-teal-dark)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Đầu Ra Rõ Ràng
            </span>
            <h2 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--color-navy)', marginTop: '0.35rem' }}>
              Bạn Sẽ Nhận Được Những Gì Khi Nghiệm Thu?
            </h2>
            <p style={{ fontSize: '0.95rem', color: 'var(--color-text-muted)' }}>
              Mọi hạng mục công việc đều được đo lường và bàn giao đầy đủ tài khoản quản trị chính chủ.
            </p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {service.deliverables.map((item, idx) => (
              ...
              >
                <div style={{ width: 26, height: 26, borderRadius: '50%', backgroundColor: 'var(--color-teal-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                  <Check size={15} color="var(--color-teal-dark)" />
                </div>
                <span style={{ fontSize: '0.925rem', color: 'var(--color-navy)', fontWeight: 600, lineHeight: 1.5 }}>
                  {item}
                </span>
              </div>
            ))}
        ...
        <div style={{ marginBottom: '3.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem' }}>
            {service.process.map((step) => (
              ...
              >
                <span style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--color-teal-dark)' }}>
                  {step.step}
                </span>
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {service.requirements.map((req, idx) => (
                <li key={idx} style={{ fontSize: '0.9rem', color: 'var(--color-navy)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {proofCaseStudy && (
          ...
          >
            ...
            <h3 style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--color-navy)', marginTop: '0.75rem', marginBottom: '0.75rem' }}>
        ...
        {service.faqs && service.faqs.length > 0 && (
          <div style={{ marginBottom: '3.5rem' }}>
            <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-navy)', textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {service.faqs.map((faq, idx) => (
                ...
                >
                  <h4 style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--color-navy)', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <HelpCircle size={18} color="var(--color-teal-dark)" /> {faq.question}
                  </h4>
        ...
        {relatedArticles.length > 0 && (
          <div style={{ marginBottom: '3.5rem' }}>
            <h3 style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--color-navy)', marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
              {relatedArticles.map((art) => (
                ...
                >
                  <div>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-teal-dark)' }}>{art.category}</span>
                    <h4 style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--color-navy)', marginTop: '0.35rem', lineHeight: 1.4 }}>
                      {art.title}
                    </h4>
                  </div>
                  <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--color-teal-dark)', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        >
          ...
          <p style={{ fontSize: '1rem', color: 'rgba(255, 255, 255, 0.85)', maxWidth: '650px', margin: '0 auto 2rem auto', lineHeight: 1.6 }}>
            Bàn giao nghiệm thu tính năng thực tế rồi mới thanh toán. 100% tài khoản chính chủ bàn giao cho bạn.
          </p>
